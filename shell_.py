...
st.title("Digital Twin: Building (Full Upper Floor Update)")
...
# --- 10. FIRST FLOOR (NEW UPPER ROOMS) ---
TAB_Z_START, TAB_Z_END = SLAB_TOP, SLAB_TOP + 2.50
# Outer perimeter
add_3d_wall(fig, [R2_X_END, WEST_LIMIT_X], [SLAB_Y_N, SLAB_Y_N+T], [TAB_Z_START, TAB_Z_END], "FF N Perimeter", TAB_C)
add_3d_wall(fig, [WEST_LIMIT_X, WEST_LIMIT_X+T], [SLAB_Y_N, SLAB_Y_S], [TAB_Z_START, TAB_Z_END], "FF W Perimeter", TAB_C)
add_3d_wall(fig, [R5_XE, WEST_LIMIT_X], [SLAB_Y_S-T, SLAB_Y_S], [TAB_Z_START, TAB_Z_END], "FF S Perimeter", TAB_C)
add_3d_wall(fig, [R5_XE-T, R5_XE], [R5_YN, SLAB_Y_S], [TAB_Z_START, TAB_Z_END], "FF E Perimeter", TAB_C)

# --- FIRST FLOOR INTERIOR LAYOUT ---
# 1. Kitchen (North-West Corner)
add_3d_wall(fig, [2.0, WEST_LIMIT_X], [0, 3.5], [TAB_Z_START, TAB_Z_START+0.05], "FF Kitchen Floor", KITCHEN_C)
add_3d_wall(fig, [2.0, 2.0+T], [0, 3.5], [TAB_Z_START, TAB_Z_END], "Kitchen-Hall Wall", "white")

# 2. Bedroom 1 (South-West Corner)
add_3d_wall(fig, [2.0, WEST_LIMIT_X], [R3_Y_DIVIDE, SLAB_Y_S-T], [TAB_Z_START, TAB_Z_END], "Bedroom 1", "lightgrey", 0.5)

# 3. Bedroom 2 (South-East Hub Area)
add_3d_wall(fig, [R5_XE, 0], [R5_YN, SLAB_Y_S], [TAB_Z_START, TAB_Z_END], "Bedroom 2", "lightgrey", 0.5)

# 4. Bedroom 3 (Near Stair Arrival)
add_3d_wall(fig, [R2_X_END, 0], [SLAB_Y_N, 2.0], [TAB_Z_START, TAB_Z_END], "Bedroom 3", "lightgrey", 0.5)

# 5. Wet Area: 2 Bathrooms + 1 Toilet (Central West)
WET_Y_START, WET_Y_END = 3.5, 6.5
add_3d_wall(fig, [3.0, WEST_LIMIT_X], [WET_Y_START, WET_Y_END], [TAB_Z_START, TAB_Z_START+0.05], "Wet Area Floor", BATH_C)
# Internal splits for 2 baths and 1 toilet
add_3d_wall(fig, [3.0, WEST_LIMIT_X], [4.5, 4.5+T/2], [TAB_Z_START, TAB_Z_END], "Bath 1 Split", "white")
add_3d_wall(fig, [3.0, WEST_LIMIT_X], [5.5, 5.5+T/2], [TAB_Z_START, TAB_Z_END], "Bath 2 Split", "white")
add_3d_wall(fig, [3.0, 3.0+T], [WET_Y_START, WET_Y_END], [TAB_Z_START, TAB_Z_END], "Wet Area Corridor Wall", "white")

# --- Final view ---
fig.update_layout(
    scene=dict(
        aspectmode='data', 
        camera=dict(eye=dict(x=-1.8, y=-1.8, z=2.0)),
